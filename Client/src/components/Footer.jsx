...
const API = {
  auth: {
    profile: `${(import.meta.env.VITE_API_BASE_URL || "http://localhost:5000").replace(/\/$/, "")}/api/auth/profile`,
  },
};

// Toast component
const Toast = ({ message, onClose, type = "error", duration = 3000, onNavigate }) => {
  const [visible, setVisible] = useState(false);
  const [progress, setProgress] = useState(100);

  useEffect(() => {
    setVisible(true);
    const interval = setInterval(() => setProgress((prev) => Math.max(0, prev - (100 / duration) * 50)), 50);
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center z-50 pointer-events-none">
      ...
      >
        ...
        <div className="mb-6">
          <div className={`text-6xl mb-4 ${type === "error" ? "text-red-500" : "text-green-500"}`}>
            {type === "error" ? "⚠️" : "✅"}
          </div>
          <h3 className="text-2xl font-bold text-black mb-2">{type === "error" ? "Login Required" : "Success!"}</h3>
          <p className="text-gray-700 text-lg">{message}</p>
        </div>

        <div className="w-full bg-gray-200 rounded-full h-2.5 mt-6">
          <div className={`h-2.5 rounded-full ${type === "error" ? "bg-red-500" : "bg-green-500"}`} style={{ width: `${progress}%` }}></div>
        </div>
...
const Footer = () => {
  ...
  const getPhotoUrl = (photoURL) => {
    ...
    return `${(import.meta.env.VITE_API_BASE_URL || "http://localhost:5000").replace(/\/$/, "")}/uploads/${photoURL}`;
  };
    ...
    <>
      <footer className="fixed bottom-0 left-0 right-0 bg-gradient-to-r from-blue-800/90 via-emerald-700/95 to-green-800/95 backdrop-blur-lg py-2 px-6 shadow-lg border-t border-white/10 z-40">
        <div className="flex justify-around items-center">
          <button onClick={() => navigate("/")} className="flex flex-col items-center text-white/90 hover:text-white transition-colors p-2">
            <FiHome size={20} className="mb-1" />
            <span className="text-xs">Home</span>
          </button>

          <button onClick={() => navigate("/search")} className="flex flex-col items-center text-white/90 hover:text-white transition-colors p-2">
            <FiSearch size={20} className="mb-1" />
            <span className="text-xs">Search</span>
          </button>

          <button onClick={() => navigate("/create")} className="flex flex-col items-center mt-0">
          ...
          <button onClick={() => navigate("/messages")} className="flex flex-col items-center text-white/90 hover:text-white transition-colors p-2 relative">
      ...
      {toast && <Toast message={toast} onClose={() => setToast(null)} type="error" onNavigate={() => navigate("/login")} />}
    </>
  );
};
